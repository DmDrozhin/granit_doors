<template>
  <button class="ui-button-order order-btn" :style="{ 'background-color': crnt.bg }" @click="clicked">
    <div class="order-btn__wrapper">
      <div class="order-btn__side-l l">
        <div class="l__svg-bg">
          <svg 
...
.order-btn {
  ...
  &__side-r, .r {
    ...
    &__txt {
      text-align: left;
      @include button-txt;
      @include media('min', 'sm') { font-size: 0.875rem; padding-left: 38px; } // 14
      @include media('min', 'lg') { font-size: 0.875rem; padding-left: 15px; } // 14
      color: #fff;
    }
  }
